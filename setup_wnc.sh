...
if [[ -z "${connectUseTBaceWNC}" ]]; then
  ...
  if [[ ${aceStatus} -eq 0 ]]; then
    ...
    f_echo "Installation of the OSG WN Client from TarBall ${aceHTTP}/download/${aceWNCtb}"
    ...
    wget --quiet --directory-prefix=${aceWNC} ${aceHTTP}/download/${aceWNCtb}
